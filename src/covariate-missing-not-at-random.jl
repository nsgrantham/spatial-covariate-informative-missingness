...
# under the assumption that its values are missing-not-at-random, or, 
# put another way, informatively-missing

function get_model(::Type{MissingNotAtRandom}, monitor::Dict{Symbol, Any}, hyper::Dict{Symbol, Any})
    model = Model(
        y = Stochastic(2,
...
function get_inits(::Type{MissingNotAtRandom}, params::Dict{Symbol, Any}, data::Dict{Symbol, Any})
    y_init = copy(data[:y])
